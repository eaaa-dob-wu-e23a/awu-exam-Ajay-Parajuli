    ...
    return (
        <>
            <div className="mt-15 p-5 xl:flex xl:flex-col xl:justify-center shadow-2xl xl:items-center xl:shadow-none xl:mt-[50px] xl:w-full xl:h-[80vh]">
                <h1 className="p-3 text-secondary font-medium text- text-2xl">Profile</h1>
                <div className="flex xl:flex-row flex-col bg-background xl:shadow-2xl xl:w-[40%] xl:max-h-[300px] xl:p-5 border-b-2 border-border ">
                    ...
                    <div className="flex flex-col justify-around mt-2 xl:ml-5">
                        ...
                        <p className="text-secondary text-sm">Languages: {user.languages.map((language, index) => (
                            <span key={language}>
                        ...
                        <div className="flex justify-between mt-5 w-full">
                            <Link to="/update-profile" className="bg-btnone p-2 rounded text-primary">Edit profile</Link>
                            <Form method="post">
                                <button className="bg-btntwo p-2 rounded text-primary">Logout</button>
                            </Form>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col justify-start xl:h-[400px] overflow-y-auto border-b-4 border-border rounded">
                        {myevents.map((event) => (
                            <div key={event._id} className="flex justify-between items-center border-border p-2 border-b-2">
                                <div>
                                    <h3 className="font-medium text-xl text-secondary">{event.title}</h3>
                                    <p className="text-secondary">{event.description}</p>
                                    <p className="text-secondary text-sm">Date: {format(new Date(event.date), "yyyy-MM-dd")}</p>
                                </div>
                                <Link to={`/events/${event._id}`} className="bg-btnone p-2 rounded text-primary">View</Link>
                            </div>
                        ))}
                    </div>
                </div>
