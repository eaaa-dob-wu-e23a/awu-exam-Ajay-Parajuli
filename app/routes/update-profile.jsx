...
export default function UpdateProfile() {
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full md:h-[95vh] xl:h-[90vh] ">
      <Form
        className="flex flex-col shadow-2xl p-4 rounded-xl w-[95%] sm:w-[85%] md:w-[70%] lg:w-[60%] xl:w-[50%] border-b-2 border-border border-l-2"
        id="sign-up-form"
        method="post"
      >
        <h2 className="border-border mb-4 pb-3 border-b-2 font-medium text-secondary text-xl">
          Update Profile
        </h2>
        <div></div>
        ...
        <div className="bg-btntwo mt-3 rounded w-full text-center">
        ...
        <div className="flex justify-between mt-2 w-full text-white">
          <button
            className="bg-btnone p-2 rounded text-primary text-lg"
            type="submit"
          >
            Update
          </button>
...
export async function action({ request }) {
  ...
  try {
    ...
    userToUpdate.languages = userData.languages
      .split(",")
      .map((lang) => lang.trim()); // Split by comma and remove extra spaces
